...
public class QuoraProblemFast
{
	int[][] matrix;
	int row, width;
	AdjacencyMatrixFast graph;
	...
	public static void main(String[] args)
	{
		...
		QuoraProblemFast r = new QuoraProblemFast(Integer.valueOf(args[0]), Integer.valueOf(args[1]), args);
		r.generate();
		stopWatch.stop();
...
//		r.printGraph();
	...
	public QuoraProblemFast(int r, int w, String[] data)
	{
//		System.out.println(Arrays.toString(data));
	...
	private void listPaths(String start, String finish)
	{
		...
		FastTable<Entry> traversalList = new FastTable<Entry>();
		FastTable<FastTable<String>> pathList = new FastTable<FastTable<String>>();
		FastMap<String, String> currentNodesInTraversalList = new FastMap<String, String>();
		...
		currentNodesInTraversalList.put(start, null);
		
		int pathLengthSize = graph.size();
		System.out.println("Size = " + pathLengthSize);
		while(traversalList.size() != 0)
		{
//			printTraversalList(traversalList, currentNodesInTraversalList);
...
//			System.out.println("List : " + tempEntry.getNodeName() + "   Popped Item : " + nextNode);
			...
			FastTable<String> a1 = getAdjacentList(nextNode);
...
//				System.out.println("**** Path Found *****");
				...
				currentNodesInTraversalList.put(SINK_NODE_LABEL, null);
//				printTraversalList(traversalList, currentNodesInTraversalList);
				...
				pruneTraversalList(traversalList, currentNodesInTraversalList);
...
//				if ( ! existsInTraversalList(n, traversalList) )
				if ( ! currentNodesInTraversalList.containsKey(n) )
				{
					if ( ! n.equals(SINK_NODE_LABEL) )			
						adjacent1.push(n);
...
//				System.out.println("adjacent1.size() : " + adjacent1.size());
			...
			else
			{
				...
				traversalList.add(entry1);
				currentNodesInTraversalList.put(nextNode, null);
			}
		}
...
//		prinAllPaths(pathList);
	...
	private void prinAllPaths(FastTable<FastTable<String>> p)
	{
	...
	private void addAndPrintPathFound(FastTable<Entry> l, FastTable<FastTable<String>> pathList)
	{
		FastTable<String> path = new FastTable<String>();
...
//		System.out.println();
	...
	private void pruneTraversalList(FastTable<Entry> l, FastMap<String, String> m)
	{
		int index = l.size() - 1;
		while(true)
		{
...
//			System.out.println("Entry : " + e.toString());
	...
	private void printTraversalList(FastTable<Entry> l, FastMap<String, String> m)
	{
		...
		System.out.println("----------------");
		System.out.println("Current Traversal List - " + m);
		
	}
	
	private	boolean existsInTraversalList(String node, FastTable<Entry> traversalList)
	{
	...
	private FastTable<String> getAdjacentList(String node)
	{
